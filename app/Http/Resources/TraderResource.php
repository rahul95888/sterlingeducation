...
class TraderResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        return [
            ...
            //'process_method_uid' => new ProcessMethodResource($this->whenLoaded('process_method_uid')),
            //'process_capability_uid' => new ProcessCapabilityResource($this->whenLoaded('process_capability_uid')),
            'user_crop_details' => TraderCropDetailResource::collection($this->whenLoaded('userCropDetails')),
           // 'process_methods' => ProcessMethod::getProcessMethodList($this->process_method_uid),
         //  'process_capabilities' => ProcessCapability::getProcessCapabilityList($this->process_capability_uid),
            // 'mandi_registration_details' => UserCropDetail::getMandiRegistrationDetail()
        ];
    }
}
