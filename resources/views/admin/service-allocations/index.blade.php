                ...
                <div class="table-responsive">
                    <table id="service_allocations_table" class="table table-striped table-bordered" style="width:100%">
                        ...
                        @php($i = "1")
                            ...
                            <td><a class="btn btn-primary btn-sm me-3" title="Edit All Location Details" href="{{ route('service-allocations.edit', $data->id) }}">
                                        <i class="bx bx-edit"></i>
                                    </a>
                                    <button class="btn btn-danger btn-sm deleteItem" title="Delete All Location" data-id="{{$data['id']}}">
                                        <i class="bx bx-trash"></i>
                                    </button>
                                    <form id="deleteForm{{$data->id}}" action="{{route('service-allocations.destroy', [$data->id])}}" method="post" style="display:none">
                                        @csrf
                                        @method('delete')
...
<script>
    $(document).ready(function(){
        $('#service_allocations_table').DataTable({
