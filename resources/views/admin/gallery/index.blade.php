                        ...
                        @php($i = "1")
                            ...
                            <td>
                                <?php if($data->image != NULL){
                                    ?>
                                    <img src="/uploads/property/{{$data->image }}" style="width:40px; height:40px;" class="img-fluid thumb-img" />  
                                     <?php
                                }?></td>
                            <td><a class="btn btn-primary btn-sm me-3" title="Edit POP Details" href="{{ route('gallery.edit', $data->id) }}">
                                        <i class="bx bx-edit"></i>
                                    </a>
                                    <button class="btn btn-danger btn-sm deleteItem" title="Delete POP" data-id="{{$data['id']}}">
                                        <i class="bx bx-trash"></i>
                                    </button>
                                    <form id="deleteForm{{$data->id}}" action="{{route('gallery.destroy', [$data->id])}}" method="post" style="display:none">
                                        @csrf
                                        @method('delete')
